...
DifferenceResult evaluateExpression(SingleFileExpression expression, Configuration config)
{
    std::vector<std::shared_ptr<Node>> left_side_trees{};
    for (const auto &left_side_system : expression.left_side)
    {
        std::filesystem::path full_path = config.base_path / left_side_system.fullPath();
    ...
    {
        std::filesystem::path full_path = config.base_path / right_side_system.fullPath();
        std::shared_ptr<Node> root = parse_file(full_path, config.options.language);
